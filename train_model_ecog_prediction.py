...
def prep_data(data_dict, data_suffix, batch_size, device, seq_len=None, ch_idx=None, transform=None):
    ...
    train_data_src  = torch.Tensor(data_dict['train_%s'%data_suffix])[:,:seq_len,ch_idx]
    valid_data_src  = torch.Tensor(data_dict['valid_%s'%data_suffix])[:,:seq_len,ch_idx]
    train_data_trg  = torch.Tensor(data_dict['train_%s'%data_suffix])[:,seq_len:2*seq_len,ch_idx]
    valid_data_trg  = torch.Tensor(data_dict['valid_%s'%data_suffix])[:,seq_len:2*seq_len,ch_idx]
    
    num_trials, num_steps, input_size = train_data_src.shape
    
    train_ds    = EcogTensorDataset(train_data_src,train_data_trg,device=device,transform=transform)
    valid_ds    = EcogTensorDataset(valid_data_src,valid_data_trg,device=device,transform=transform)
...
#-------------------------------------------------------------------
#-------------------------------------------------------------------
# updated for the prediction model - it's a different task!
