...
def main():
    ...
    save_loc = save_loc[:-1] + f'_reduced/'
...
def prep_data(data_dict, data_suffix, batch_size, device, n_samp=50, ch_idx=[10]):
    train_data = torch.Tensor(data_dict['train_%s'%data_suffix])[:,:n_samp,ch_idx]
    valid_data = torch.Tensor(data_dict['valid_%s'%data_suffix])[:,:n_samp,ch_idx]
